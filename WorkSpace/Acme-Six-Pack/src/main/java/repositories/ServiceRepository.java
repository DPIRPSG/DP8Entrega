...
public interface ServiceRepository extends JpaRepository<ServiceEntity, Integer> {
	...
	@Query("select s from ServiceEntity s group by s having s not in (select distinct(b.service) from Customer c join c.bookings b where c.id = ?1) and s in (select distinct(s) from Customer c join c.feePayments f join f.gym.services s where c.id = ?1 and f.activeMoment < CURRENT_DATE and f.inactiveMoment > CURRENT_DATE)")
